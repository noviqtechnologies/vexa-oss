...
def evaluate_quality_gate(
    ...
    if new_only or baseline_file:
        if BaselineManager:
            ...
            print_warning("Baseline logic unavailable (cicd package missing). Checking all findings.")

    if QualityGateEvaluator:
        from vexa.common.config_manager import QualityGateConfig
        evaluator = QualityGateEvaluator()
        config_kwargs = {
            "fail_on": [s.strip().lower() for s in fail_on.split(",")] if fail_on else [],
        ...
        status_text = "[bold spring_green3]PASSED[/bold spring_green3]" if gate_status.passed else "[bold red]FAILED[/bold red]"
