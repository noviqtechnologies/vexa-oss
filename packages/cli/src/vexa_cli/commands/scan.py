...
from vexa.common.models import ScanMode, JobStatus, ScanResult
from vexa.common.cloud_provider import CloudProvider, PROVIDER_CAPABILITIES, ProviderAvailability
from vexa.common.config_manager import load_config

from vexa_cli.ui.output import print_banner, print_success, print_error, print_info, print_warning, console
...
try:
    ...
    for ep in eps:
        cls = ep.load()
        if ep.name == "QualityGateEvaluator": QualityGateEvaluator = cls
        elif ep.name == "SecurityScorer": SecurityScorer = cls
        elif ep.name == "BaselineManager": BaselineManager = cls
        elif ep.name == "ComplianceAnalyzer": ComplianceAnalyzer = cls
        elif ep.name == "ShadowDecorator": ShadowDecorator = cls
except Exception:
    pass

@click.command()
@click.argument("path", type=click.Path(exists=True, path_type=Path))
@click.option("--format", "-f", type=click.Choice(["html", "json", "sarif", "markdown", "all"]), multiple=True, default=["all"], help="Output report format (default: all)")
@click.option("--output", "-o", type=click.Path(path_type=Path), default=None, help="Output directory for reports (default: 'vexa_scan_reports' in the scanned directory)")
@click.option("--scanners", "-s", multiple=True, help="Specific scanners to run (default: all available)")
@click.option("--mode", "-m", type=click.Choice(["local", "container"]), default="local", help="Execution mode (default: local)")
@click.option("--timeout", "-t", type=int, default=480, help="Timeout per scanner in seconds (default: 480)")
@click.option('--ai-provider', type=click.Choice(["google", "openai", "anthropic", "ollama", "none"], case_sensitive=False), default=None, help="AI provider override for explicit false positive filtering (e.g., 'openai', 'ollama' for local)")
@click.option("--ai-api-key", type=str, default=None, envvar='VEXA_AI_API_KEY', help="API Key for the chosen AI provider (also via VEXA_AI_API_KEY)")
@click.option("--fail-on", "-F", type=str, default=None, help="Comma-separated list of severities to fail the pipeline (e.g., 'critical,high')")
@click.option("--ai-min-severity", type=click.Choice(["critical", "high", "medium", "low", "info"]), default="medium", help="Minimum severity threshold for AI enrichment (default: medium)")
@click.option("--max-findings", type=int, default=0, help="Maximum total findings allowed before failing the quality gate (default: 0 = unlimited)")
@click.option("--new-only", is_flag=True, default=False, help="Only fail on findings that are not in the baseline file.")
@click.option("--baseline-file", type=click.Path(path_type=Path), default=None, help="Path to the baseline file (.vexa-baseline.json)")
@click.option("--min-score", type=str, default=None, help="Minimum security score grade (e.g., 'B+') required to pass.")
@click.option("--shadow", is_flag=True, default=False, help="Enable Shadow Mode: execute PR decoration silently and always exit with code 0 (Recommended default for CI/CD).")
@click.option("--decorate-pr", type=click.Choice(["github", "gitlab", "auto"]), default=None, help="Post scan findings as a PR/MR comment to the specified platform.")
@click.option("--compliance-framework", type=click.Choice(["soc2", "pci", "hipaa", "nist", "all"]), default=None, help="Map findings to the specified compliance framework.")
@click.option("--exclude", "-x", multiple=True, help="Paths to exclude from scanning (e.g. 'test/,*.md')")
@click.option("--incremental", is_flag=True, default=False, help="Only scan files modified in git (uses git diff --name-only HEAD).")
@click.option("--fast", is_flag=True, default=False, help="Run fast mode (bandit + detect-secrets only) suitable for pre-commit.")
@click.option("--pre-commit", is_flag=True, default=False, help="Run in pre-commit hook mode (enforces guardrails).")
def scan(
    path: Path, format: tuple, output: Optional[Path], scanners: tuple, mode: str, timeout: int,
    ai_provider: Optional[str] = None, ai_api_key: Optional[str] = None, fail_on: Optional[str] = None,
    ai_min_severity: str = "medium", max_findings: int = 0, new_only: bool = False,
    baseline_file: Optional[Path] = None, min_score: Optional[str] = None, shadow: bool = False,
    decorate_pr: Optional[str] = None, compliance_framework: Optional[str] = None, exclude: tuple = (),
    incremental: bool = False, fast: bool = False, pre_commit: bool = False,
):
    """
    ...
    if max_findings == 0 and config.quality_gate.max_total != 50: # 50 is the pydantic default
    ...
    if fast:
        ...
        print_info(f"Using scanners from project configuration: {', '.join(scanner_list)}")
    else:
        scanner_list = None
    ...
    if pre_commit:
        ...
        if config.ai.enabled and config.ai.provider != CloudProvider.NONE:
            ...
            print_info(f"Using AI provider '{cloud_provider.value}' from project configuration")
        ...
        if not status["available"]:
            ...
            if not is_ci and not is_non_interactive:
                ...
                if key_var and not os.environ.get(key_var):
                    ...
                    new_key = Prompt.ask(f"Enter your {cloud_provider.value.title()} API Key now", password=True)
                    if new_key:
                        os.environ[key_var] = new_key
                        # Re-check availability with the new key
                        status = asyncio.run(ProviderAvailability.check_provider(cloud_provider, check_quota=True))
                        if status["available"]:
                            print_success(f"API Key accepted for this session.")

            if not status["available"]:
                if is_ci or is_non_interactive:
                    print_warning("Non-interactive / CI environment detected. Automatically proceeding without AI enrichment.")
                    cloud_provider = CloudProvider.NONE
                    ai_provider = None
                elif Confirm.ask("Would you like to proceed with the scan without AI enrichment?", default=False, console=console):
    ...
    display_map = {"google": "GOOGLE GEMINI", "openai": "OPENAI", "anthropic": "ANTHROPIC", "none": "None"}
    effective_ai_display = display_map.get(ai_provider.lower(), ai_provider.upper()) if ai_provider else display_map.get(cloud_provider.value.lower(), "None") if cloud_provider != CloudProvider.NONE else "None"
    ...
    try:
        result = asyncio.run(_run_scan(
            path, scanner_list, scan_mode, timeout, cloud_provider,
            ai_provider_override=ai_provider, api_key_override=ai_api_key,
            ai_min_severity=ai_min_severity, cli_excludes=cli_excludes,
    ...
    if result.success:
        ...
        if SecurityScorer:
            ...
            if BaselineManager:
                try:
                    bm = BaselineManager()
                    bm.save_baseline(result, path / ".vexa-baseline.json", score_grade=score_grade)
                    console.print("   [dim]Saved score and baseline to .vexa-baseline.json[/dim]")
                except Exception as e:
        ...
        for scanner_name in (scanner_list or all_expected_scanners):
            if scanner_name in result.scanners_run:
                ...
                findings_list = getattr(scanner_res_dict, "findings", []) if not isinstance(scanner_res_dict, dict) else scanner_res_dict.get("findings", [])
                fn_count = len(findings_list)
                console.print(f"  [bold green]✔[/bold green] [white]{scanner_name:<16}[/white] [dim]({fn_count} findings)[/dim]")
            else:
                if sys.platform == "win32" and scanner_name == "semgrep":
                    console.print(f"  [bold red]✖[/bold red] [white]{scanner_name:<16}[/white] [dim](not available on Windows)[/dim]")
                else:
                    console.print(f"  [bold yellow]⬚[/bold yellow] [white]{scanner_name:<16}[/white] [dim](not installed/available)[/dim]")

        gate_failed, diff, gate_status = evaluate_quality_gate(
            result, score_grade, new_only, baseline_file, path, fail_on, max_findings, min_score,
            QualityGateEvaluator, BaselineManager
        )
    ...
    console.print("[dim italic cyan]💡 Have feedback? Run 'vexa feedback' to help us improve![/dim italic cyan]\n")

    comp_report = None
    if compliance_framework and QualityGateEvaluator:
        if ComplianceAnalyzer:
            analyzer = ComplianceAnalyzer()
            frames = ["soc2", "pci-dss", "hipaa", "nist"] if compliance_framework == "all" else [compliance_framework]
    ...
    if decorate_pr and QualityGateEvaluator:
        if ShadowDecorator:
            ...
            if decorator.post_or_update_summary(markdown):
                ...
                print_warning("PR Decoration failed or was skipped (Shadow Mode prevents crash).")
        else:
            print_warning("Cannot decorate PR: cicd package components missing.")

    if shadow:
        print_info("Shadow Mode explicitly enabled. Suppressing exit code to keep pipeline green.")
        sys.exit(0)
    elif pre_commit:
        block_on_sev = config.guardrails.block_on.lower() if hasattr(config, "guardrails") else "critical"
        weight = {"critical": 4, "high": 3, "medium": 2, "low": 1}
        threshold_weight = weight.get(block_on_sev, 4)
        
        blocking_findings = []
        for sev in ["critical", "high", "medium", "low"]:
            if weight.get(sev, 0) >= threshold_weight and severity_counts.get(sev, 0) > 0:
                blocking_findings.append(sev)
                break
                
        if blocking_findings:
            highest_sev = blocking_findings[0]
            print_error(f"Commit blocked: Your code contains a {highest_sev} severity finding. Fix it and try again.")
            sys.exit(1)
            
        print_success("Pre-commit guardrails passed.")
        sys.exit(0)
    elif gate_failed:
        print_error(f"Quality Gate Failed: Found findings matching thresholds.")
        sys.exit(1)


async def _run_scan(
    path: Path, scanners: Optional[List[str]], mode: ScanMode, timeout: int, cloud_provider: CloudProvider,
    ai_provider_override: Optional[str] = None, api_key_override: Optional[str] = None,
    ai_min_severity: str = "medium", cli_excludes: Optional[List[str]] = None,
    ...
    scan_job_id = await run_scan_with_job_manager(
        engine, job_manager, path, scanners, mode, timeout,
        cloud_provider, ai_min_severity=ai_min_severity,
        cli_excludes=cli_excludes, incremental=incremental,
    )
