...
from vexa.common.config import is_terms_accepted
...
def detect_project_type():
    ...
    if any(
        os.path.exists(f)
        for f in ["requirements.txt", "pyproject.toml", "setup.py", "uv.lock"]
    ):
        types.append("Python")
    if any(
        os.path.exists(f) for f in ["package.json", "tsconfig.json", "node_modules"]
    ):
    ...
    if any(
        os.path.exists(f) for f in ["Dockerfile", "docker-compose.yml", "Containerfile"]
    ):
...
def install_scanners(scanners_to_install):
    ...
    pip_installable = [
        s
        for s in scanners_to_install
        if s not in ("npm-audit", "syft", "grype", "trivy")
    ]
    ...
    if sys.version_info >= (3, 14):
        console.print(
            "[yellow]Warning: Python 3.14+ detected. Many security tools (like Semgrep) have dependency conflicts on this version.[/yellow]"
        )
        console.print(
            "[cyan]Recommendation: Use a stable version (Python 3.11/3.12) or install tools via 'pipx' for isolation.[/cyan]\n"
        )

    if Confirm.ask(
        f"Would you like to auto-install recommended scanners ({', '.join(pip_installable)})?",
        default=True,
        console=console,
    ):
        with console.status(
            "[bold green]Installing scanners...[/bold green]", spinner="dots"
        ):
            ...
            if failed:
                console.print(
                    f"[yellow]Failed to install some scanners: {', '.join(failed)}[/yellow]"
                )
                console.print(
                    "[dim]This is often due to Python version mismatches. Try installing them manually or using 'pipx'.[/dim]"
                )
            else:
...
def configure_ai():
    if Confirm.ask(
        "Would you like to use AI-powered analysis for more accurate results?",
        default=True,
        console=console,
    ):
        ...
        if provider == "ollama":
            ...
            console.print(
                "[dim]Ensure Ollama is running (`ollama serve`) and the model is pulled (`ollama pull llama3:8b`).[/dim]\n"
            )
            return {
                "enabled": True,
                "provider": "ollama",
        ...
        # Prompt for API Key
        api_key = Prompt.ask(
            f"Enter your {provider.capitalize()} API Key (leave empty to configure later)",
            password=True,
            console=console,
        )
        ...
        if api_key:
            ...
            console.print(
                "[dim]Note: This key is active for the current session. For permanent access, add it to your environment:[/dim]"
            )
            ...
            if sys.platform == "win32":
                console.print(
                    f'  [cyan]PowerShell:[/cyan] [white]$env:{key_var}="{masked_key}"[/white]'
                )
            else:
                console.print(
                    f'  [cyan]Bash/Zsh:[/cyan]   [white]export {key_var}="{masked_key}"[/white]'
                )
            ...
            if sys.platform == "win32":
                console.print(
                    f'  [cyan]PowerShell:[/cyan] [white]$env:{key_var}="YOUR_KEY"[/white]'
                )
                console.print(
                    f"  [cyan]CMD:[/cyan]        [white]set {key_var}=YOUR_KEY[/white]"
                )
            else:
                console.print(
                    f'  [cyan]Bash/Zsh:[/cyan]   [white]export {key_var}="YOUR_KEY"[/white]'
                )

            console.print(
                f"[dim]Obtain your key at the {provider.capitalize()} developer portal.[/dim]\n"
            )
        ...
        if missing_sdk:
            ...
            console.print(
                "[white]Vexa uses optional AI dependencies to keep your standard CI/CD drops blazing fast without bloat.[/white]"
            )
            console.print(
                "Please run the following command to install the required Python SDK before running 'vexa fix':"
            )
...
def generate_config(ai_config, scanner_list):
    ...
    config_data = {
        ...
        "ai": ai_config,
        "reports": {"formats": ["sarif", "html", "json", "markdown"]},
    }
...
def _install_guardrails():
    ...
    if not git_hooks_dir.exists():
        print_warning(
            "No .git/hooks directory found. Ensure you are in the root of a Git repository."
        )
        return
...
"""
    try:
        if hook_path.exists():
            if Confirm.ask(
                f"[yellow]{hook_path} already exists. Overwrite with Vexa guardrails?[/yellow]",
                default=False,
                console=console,
            ):
