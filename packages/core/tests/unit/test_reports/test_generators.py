...
def sample_scan_result(sample_findings):
    ...
    return ScanResult(
        ...
        scanner_results={
            "bandit": ScannerResult(
                scanner_name="bandit", success=True, findings=[sample_findings[0]]
            ),
            "semgrep": ScannerResult(
                scanner_name="semgrep", success=True, findings=[sample_findings[1]]
            ),
            "checkov": ScannerResult(
                scanner_name="checkov", success=True, findings=[sample_findings[2]]
            ),
        },
...
class TestSARIFReportGenerator:
    ...
    def test_sarif_ai_metadata(self, sample_scan_result, tmp_path):
        ...
        bandit_run = next(
            r for r in data["runs"] if r["tool"]["driver"]["name"] == "bandit"
        )
        semgrep_run = next(
            r for r in data["runs"] if r["tool"]["driver"]["name"] == "semgrep"
        )
