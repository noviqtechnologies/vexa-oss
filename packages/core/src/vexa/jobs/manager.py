...
class JobManager:
    ...
    def __init__(self):
        ...
        logger.info("JobManager initialized with max_concurrent=%d, ttl_hours=%d",
                   self.MAX_CONCURRENT, self.JOB_TTL_HOURS)
    ...
    async def _run_job(self, job_id: str, task: Coroutine) -> None:
        ...
        async with self._semaphore:
            try:
                ...
                audit_logger.log_job_event(job_id, "completed", {"result_type": type(result).__name__})
    ...
    async def update_progress(self, job_id: str, progress: float) -> None:
        ...
        logger.debug("Job progress updated: id=%s, progress=%.1f%%", job_id, clamped_progress)
    ...
    async def get_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            state = job.status.upper() if isinstance(job.status, str) else job.status.value.upper()
    ...
    async def update_job(
        ...
        logger.debug(
            ...
            job_id, clamped_progress, len(findings) if findings else 0
        )
        
        # Helper to get status string safe for logging
        status_val = status.value if hasattr(status, 'value') else status
        
        audit_logger.log_job_event(job_id, "updated", {
