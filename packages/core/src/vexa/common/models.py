...
class Job(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=utc_now, description="Job creation timestamp"
    )
    ...
    intermediate_findings: List[Any] = Field(
        default_factory=list, description="Intermediate findings during processing"
    )
...
class Finding(BaseModel):
    ...
    nist_controls: List[str] = Field(
        default_factory=list, description="NIST control mappings"
    )
...
class EnhancedFinding(Finding):
    ...
    detailed_description: str = Field(
        default="", description="AI-generated detailed description"
    )
    ...
    implementation_steps: List[str] = Field(
        default_factory=list, description="Step-by-step fix"
    )

    # Cloud provider specific recommendations
    google_cloud_recommendation: str = Field(
        default="", description="GCP-specific recommendation"
    )
    google_cloud_doc_links: List[str] = Field(
        default_factory=list, description="Google Cloud documentation links"
    )
    ...
    cli_commands: List[str] = Field(
        default_factory=list, description="Implementation CLI commands"
    )
    test_cases: List[str] = Field(
        default_factory=list, description="Verification test cases"
    )
...
class Threat(BaseModel):
    ...
    implementation_guidance: str = Field(
        default="", description="Implementation guidance - TM-007"
    )
...
class ThreatModel(BaseModel):
    ...
    generated_at: datetime = Field(
        default_factory=utc_now, description="Generation timestamp"
    )
    cloud_provider: CloudProvider = Field(
        ..., description="Cloud provider used for analysis"
    )
    threats: Dict[str, List[Threat]] = Field(
        default_factory=dict, description="Category -> Threats mapping"
    )
    architecture_diagram: str = Field(
        default="", description="Mermaid format diagram - TM-006"
    )
    action_plan: List[Dict[str, Any]] = Field(
        default_factory=list, description="Action items - TM-007"
    )
...
class DriftProperty(BaseModel):
    ...
    severity: Literal["high", "medium", "low", "info"] = Field(
        ..., description="Drift severity - SV-005"
    )
...
class DriftFinding(BaseModel):
    ...
    drift_type: Literal["modified", "deleted", "unmanaged"] = Field(
        ..., description="Type of drift"
    )
    properties: List[DriftProperty] = Field(
        default_factory=list, description="Drifted properties"
    )
    remediation: str = Field(
        default="", description="Remediation recommendation - SV-006"
    )
...
class ScanResult(BaseModel):
    ...
    scanners_run: List[str] = Field(
        default_factory=list, description="Scanners that were run"
    )
    ...
    errors: List[str] = Field(
        default_factory=list, description="Any errors during scan"
    )
    warnings: List[str] = Field(
        default_factory=list, description="Any non-critical warnings during scan"
    )
    scanner_results: Dict[str, Any] = Field(
        default_factory=dict, description="Per-scanner results"
    )
