...
class QualityGateConfig(BaseModel):
    ...
    baseline: Optional[str] = Field(default=".vexa-baseline.json", description="Path to the baseline file")
...
class AIConfig(BaseModel):
    ...
    model: str = Field(default="gemma4:26b", description="Model name for Ollama local LLM")
...
class GuardrailsConfig(BaseModel):
    ...
    block_on: str = Field(default="critical", description="Minimum severity to block commits (critical, high, medium, low)")
...
class VexaConfig(BaseModel):
    ...
    ignore: List[str] = Field(default_factory=list, description="List of path patterns to ignore globally")
