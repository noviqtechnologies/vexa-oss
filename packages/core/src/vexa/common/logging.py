...
SENSITIVE_PATTERNS = [
    ...
    (
        re.compile(r'(?i)(api_key|apikey|api-key)["\']?\s*[:=]\s*["\']?[^\s"\']+'),
        r"\1=***REDACTED***",
    ),
    (
    ...
    (
        re.compile(r'(?i)(auth|authorization)["\']?\s*[:=]\s*["\']?[^\s"\']+'),
        r"\1=***REDACTED***",
    ),
    (
        re.compile(r'(?i)(aws_access_key|aws_secret)["\']?\s*[:=]\s*["\']?[^\s"\']+'),
        r"\1=***REDACTED***",
    ),
]
...
class AuditLogger:
    ...
    def log_job_event(
        self, job_id: str, event: str, details: Optional[Dict[str, Any]] = None
    ) -> None:
        """
