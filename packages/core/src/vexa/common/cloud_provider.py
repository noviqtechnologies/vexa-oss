...
class ProviderAvailability:
    ...
    @staticmethod
    async def check_provider(provider: CloudProvider, check_quota: bool = False) -> Dict[str, Any]:
        """
        ...
        if provider in (CloudProvider.GOOGLE, CloudProvider.OPENAI, CloudProvider.ANTHROPIC):
            ...
            if not p:
                return {"available": False, "authenticated": False, "error": f"Provider {provider.value} not configured"}
        ...
        if not capabilities or not capabilities.ai_cli_command:
             return {"available": False, "authenticated": False, "error": f"Provider '{provider.value}' is disabled or requires an SDK."}
        ...
        if capabilities.check_args:
            ...
            try:
                ...
                if os.name == 'nt' and full_path.lower().endswith(('.cmd', '.ps1')):
                    ...
                    js_path = os.path.join(wrapper_dir, "node_modules", "@google", "gemini-cli", "dist", "index.js")
                    
                    if provider == CloudProvider.GOOGLE and node_path and os.path.exists(js_path):
        ...
        if check_quota and capabilities.ping_args:
            ...
            try:
                ...
                if os.name == 'nt' and full_path.lower().endswith(('.cmd', '.bat', '.ps1')):
                    node_path = shutil.which("node")
                    wrapper_dir = os.path.dirname(full_path)
                    js_path = os.path.join(wrapper_dir, "node_modules", "@google", "gemini-cli", "dist", "index.js")
                ...
                if result.returncode != 0:
                    stderr_content = result.stderr.strip() or result.stdout.strip()
                    if any(kw in stderr_content.lower() for kw in ["429", "quota", "rate limit", "exhausted"]):
                        return {
                            "available": False,
                            "authenticated": True,
