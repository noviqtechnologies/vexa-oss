    ...
    def _send_request(self, endpoint: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            req = urllib.request.Request(
                url, data=json_data, headers=self.headers, method="POST"
            )
            ...
            with response:
                if response.status in (200, 201):
                    ...
                    logger.warning(
                        f"Telemetry request failed: {response.status} {response.reason}"
                    )
                    return False
            ...
            if "getaddrinfo failed" in str(e) or "[Errno 11002]" in str(e):
                logger.warning(
                    f"Network error: DNS resolution failed for {self.BASE_URL}. "
