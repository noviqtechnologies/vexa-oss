...
from vexa.security.validators import PathValidator, CommandSanitizer
...
class FindingValidator:
    ...
    def validate_finding(self, finding: Finding) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if ".." in finding.file_path:
                errors.append(
                    f"SEC-001: Path traversal detected in file_path: {finding.file_path}"
                )
            if "\x00" in finding.file_path:
                errors.append("SEC-001: Null byte detected in file_path")
        ...
        if finding.line_end < finding.line_start:
            errors.append(
                f"line_end ({finding.line_end}) < line_start ({finding.line_start})"
            )
    ...
    def sanitize_finding_output(self, finding: Finding) -> Finding:
        ...
        for pattern, replacement in self.SENSITIVE_PATTERNS:
            sanitized.code_snippet = re.sub(
                pattern, replacement, sanitized.code_snippet
            )
