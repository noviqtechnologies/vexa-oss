...
class OutputSanitizer:
    ...
    SENSITIVE_PATTERNS: List[Tuple[Pattern, str]] = [
        ...
        (
            re.compile(
                r'(?i)(authorization|auth)(["\']?\s*[:=]\s*["\']?)(Bearer\s+)?([^\s"\']+(?:\.[^\s"\']+)*)',
                re.IGNORECASE,
            ),
            r"\1\2***REDACTED***",
        ),
        ...
        (
            re.compile(
                r"(?i)(mongodb|mysql|postgres|redis)://[^:]+:([^@]+)@", re.IGNORECASE
            ),
            r"\1://***:***REDACTED***@",
        ),
    ]
    ...
    JSON_SENSITIVE_PATTERNS: List[Tuple[Pattern, str]] = [
        ...
        (
            re.compile(
                r'"(password|passwd|pwd|secret|api[_-]?key|token|auth)":\s*"([^"]+)"',
                re.IGNORECASE,
            ),
            r'"\1": "***REDACTED***"',
        ),
