...
class DataMerger:
    ...
    def merge_findings(
        ...
        for scanner_name, findings in findings_by_scanner.items():
            self.logger.info(
                "Processing %d findings from %s", len(findings), scanner_name
            )
        ...
        self.logger.info(
            ...
            len(all_findings),
            len(findings_by_scanner),
            list(findings_by_scanner.keys()),
        )

        # Validate findings if enabled
        if self._validate:
            all_findings, validation_errors = (
                self._security_validator.validate_findings_pre_merge(all_findings)
            )
