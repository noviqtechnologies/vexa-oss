...
class ScannerEngine:
            ...
            results.append(
                {
                    ...
                    "languages": scanner_cls.supported_languages
                    if hasattr(scanner_cls, "supported_languages")
                    else [],
                }
            )
        return results
    ...
    async def run_scan_with_progress(
        ...
        if path.is_file():
            ...
            if len(scanners) < original_count:
                self.logger.info(
                    f"Single file scan detected. Skipping workspace-level scanners: {workspace_scanners & set(scanners if isinstance(scanners, list) else [])}"
                )

        # Initial progress
        if progress_callback:
            await progress_callback(
                0, f"Initializing {len(scanners)} scanners (timeout: {timeout}s)..."
            )
        ...
        for scanner_name in scanners:
            ...
            if mode not in scanner_cls.supported_modes:
                self.logger.warning(
                    f"Scanner {scanner_name} not supported in {mode.value} mode"
                )
                warnings.append(
                    f"Scanner {scanner_name} not supported in {mode.value} mode"
                )
            ...
            try:
                ...
                if scanner.is_available():
                    ...
                    self.logger.warning(
                        f"Scanner {scanner_name} is not available on this system, skipping."
                    )
                    instructions = {
        ...
        if not scanner_instances:
            ...
            return ScanResult(
                ...
                warnings=warnings
                + (
                    ["No valid scanners available for the selected mode/platform"]
                    if not errors
                    else []
                ),
        ...
        if progress_callback:
            await progress_callback(
                10, f"Running {len(scanner_instances)} tools in parallel..."
            )
        ...
        async def run_wrapper(scanner, idx):
            ...
            try:
                ...
                scanner_timeout = int(
                    timeout * getattr(scanner, "timeout_multiplier", 1.0)
                )
                result = await scanner.run(
                    path, timeout=scanner_timeout, exclusions=exclusions
                )
                self.logger.info(
                    f"Completed {scanner.name} with {len(result.findings)} findings"
                )

                # Update progress
                completed_scanners += 1
                completion_pct = 10 + (
                    completed_scanners / total_scanners * 60
                )  # 10% to 70%
                if progress_callback:
                    await progress_callback(
                        completion_pct,
        ...
        for name, result in results:
            if isinstance(result, Exception):
                ...
                scanner_results[name] = ScannerResult(
                    scanner_name=name, success=False, error=str(result)
                )
            else:
                scanner_results[name] = result
                if result.success:
                    self.logger.info(
                        f"Adding {len(result.findings)} findings from {name}"
                    )
                else:
        ...
        self.logger.info(
            f"Total merged findings before filtering: {len(merged_findings)}"
        )
        ...
        for f in merged_findings:
            if not self._is_finding_commented(path, f):
                ...
                self.logger.info(
                    f"Filtered finding {f.id} ({f.title}) as it resides on a commented or reference line"
                )

        merged_findings = active_findings
        self.logger.info(
            f"Total merged findings after filtering: {len(merged_findings)}"
        )
        ...
        if incremental:
            try:
                ...
                norm_modified = set(
                    m.strip().replace("\\", "/").lstrip("./")
                    for m in (git_diff.splitlines() + git_untracked.splitlines())
                    if m.strip()
                )
                ...
                for finding in merged_findings:
                    ...
                    if f_path in norm_modified or any(
                        f_path.endswith("/" + m) for m in norm_modified
                    ):
                        incremental_findings.append(finding)

                self.logger.info(
                    f"Incremental scan filtered findings from {len(merged_findings)} down to {len(incremental_findings)}"
                )
        ...
        if cloud_provider != CloudProvider.NONE:
            if progress_callback:
                await progress_callback(
                    80, f"Enriching findings with {cloud_provider.value} AI..."
                )
            ...
            for finding in merged_findings:
                if not finding.code_snippet and finding.file_path:
                    try:
                        ...
                        filepath = (
                            path / f_path
                            if not Path(f_path).is_absolute()
                            else Path(f_path)
                        )
                        ...
                        if filepath.exists() and filepath.is_file():
                            with open(
                                ...
                                if 1 <= finding.line_start <= len(lines):
                                    finding.code_snippet = lines[
                                        finding.line_start - 1
                                    ].strip()
                    except Exception as e:
                        self.logger.debug(
                            f"Failed to auto-populate snippet for {finding.file_path}: {e}"
                        )
            ...
            try:
                ...
                ai_manager = get_ai_manager(
                    cloud_provider, min_severity=ai_min_severity
                )

                # Enrich findings using AI manager
                self.logger.info(
                    f"Sending {len(merged_findings)} findings for AI enrichment"
                )
                enriched, fps = await ai_manager.enrich_findings(merged_findings)
                merged_findings = enriched
                self.logger.info(
                    f"AI enrichment complete, got {len(merged_findings)} valid findings and {len(fps)} false positives back"
                )

            except ImportError:
                self.logger.warning(
                    "AI Providers module not found, skipping enrichment."
                )
            except Exception as e:
        ...
        any_success = any(
            r.success for r in scanner_results.values() if isinstance(r, ScannerResult)
        )
        ...
        return ScanResult(
            ...
            false_positives=fps,
            scanner_results={
                name: r.__dict__ if hasattr(r, "__dict__") else r
    ...
    def _is_finding_commented(self, base_path: Path, finding: Finding) -> bool:
        ...
        try:
            ...
            filepath = (
                root_dir / f_path if not Path(f_path).is_absolute() else Path(f_path)
            )
            filepath = filepath.resolve()
            ...
            with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                for idx, line in enumerate(f):
                    if idx == finding.line_start - 1:
                        stripped = line.strip()
                        self.logger.debug(
                            f"Checking line {finding.line_start} content: '{stripped}'"
                        )

                        # 1. Check for Vexa [Original] marker (Highest priority)
                        if "[Original]" in stripped or "[Vexa] Fixed" in stripped:
                            self.logger.info(
                                f"Found commented/reference line {finding.line_start}"
                            )
                            return True
            ...
            self.logger.debug(
                f"Error checking if finding {finding.id} is commented: {e}"
            )
            return False
