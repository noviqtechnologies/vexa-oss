...
class BaseScanner(ABC):
    ...
    async def run(self, path: Path, timeout: Optional[int] = None, exclusions: Optional[List[str]] = None) -> ScannerResult:
        """
        ...
        logger.info("Running scanner %s on %s (mode=%s)", self.name, path, self.mode.value)
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if not result.stdout.strip():
                    ...
                    if self.name == "checkov" and "download-external-modules" in result.stderr:
                        logger.debug("Checkov failed to download external modules. Run with --download-external-modules for full context.")
                    else:
                        logger.warning(f"Scanner {self.name} returned exit code {result.returncode} with empty stdout.")
                    return ScannerResult(
                        scanner_name=self.name,
                        success=False,
                ...
                logger.debug(f"Scanner {self.name} exited with code {result.returncode} but has stdout. Proceeding to parse.")
            ...
            if result.returncode == 0 and not result.stdout.strip() and result.stderr.strip():
                logger.warning(f"Scanner {self.name} returned empty stdout but has stderr: {result.stderr[:500]}")
            ...
            logger.info(
                ...
                self.name, len(findings), execution_time
            )
    ...
    def _generate_finding_id(self, scanner: str, file_path: str, line: int, rule: str) -> str:
