...
class DeduplicationEngine:
    ...
    def _group_by_predicate(
        self, findings: List[Finding], predicate
    ) -> List[List[Finding]]:
    ...
    def _is_fuzzy_match(self, f1: Finding, f2: Finding) -> bool:
        ...
        if (
            f1.file_path.replace("\\", "/").lower()
            != f2.file_path.replace("\\", "/").lower()
        ):
            return False
        ...
        if f1.code_snippet and f2.code_snippet:
            similarity = difflib.SequenceMatcher(
                None, f1.code_snippet, f2.code_snippet
            ).ratio()
    ...
    def _is_semantic_match(self, f1: Finding, f2: Finding) -> bool:
        ...
        if (
            f1.owasp_category
            and f2.owasp_category
            and f1.owasp_category == f2.owasp_category
        ):
            cat_match = True
        ...
        title_sim = difflib.SequenceMatcher(
            None, f1.title.lower(), f2.title.lower()
        ).ratio()
    ...
    def _select_best(self, group: List[Finding]) -> Finding:
        ...
        sorted_group = sorted(
            group, key=lambda f: self.SEVERITY_PRIORITY.get(f.severity, 0), reverse=True
        )
