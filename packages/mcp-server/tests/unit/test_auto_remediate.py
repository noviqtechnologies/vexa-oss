...
async def test_auto_remediate_workspace_dry_run():
    ...
    with patch("vexa_mcp.scanner.server.get_scanner_engine") as mock_engine, \
        ...
        mock_finding = EnhancedFinding(
            id="123", scanner="bandit", title="Hardcoded Secret", 
            severity="critical", file_path="main.py", line_start=10, line_end=10,
            code_snippet="secret = '123456'", cwe_ids=["CWE-798"], description="Mock",
        ...
        mock_result = ScanResult(
            job_id="test", status="completed", progress=100,
            findings=[mock_finding], duration_seconds=1.0, scanners_run=["bandit"]
        )
        
        engine_instance = mock_engine.return_value
        engine_instance.run_scan_with_progress = AsyncMock(return_value=mock_result)
        
        output = await auto_remediate_workspace(path=".", severity_threshold="medium", dry_run=True)
...
async def test_auto_remediate_workspace_apply_fix():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("vexa_mcp.scanner.server.get_scanner_engine") as mock_engine, \
            ...
            mock_finding = EnhancedFinding(
                id="123", scanner="bandit", title="Hardcoded Secret", 
                severity="critical", file_path=str(test_file), line_start=1, line_end=1,
                code_snippet="secret = '12345'", cwe_ids=["CWE-798"], description="Mock",
                remediation_code="secret = os.environ.get('SECRET')",
                is_false_positive=False,
                detailed_description="High risk."
            )
            ...
            output = await auto_remediate_workspace(path=temp_dir, severity_threshold="medium", dry_run=False)
...
async def test_auto_remediate_workspace_safe_failure():
    ...
    with patch("vexa_mcp.scanner.server.get_scanner_engine") as mock_engine:
        engine_instance = mock_engine.return_value
        engine_instance.run_scan_with_progress.side_effect = Exception("Docker not running")
