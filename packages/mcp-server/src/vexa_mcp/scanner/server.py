...
async def run_scan_local(
    ...
    return await create_scan_job(
        path, scanners, mode, cloud_provider, cli_excludes,
        incremental=incremental,
...
async def enrich_findings(
    ...
    try:
        ...
        enhanced_models, _fps = await manager.enrich_findings(finding_models, is_workspace_scan=True)
...
async def auto_remediate_workspace(
    ...
    provider = CP.NONE
    if getattr(ai_manager, "_primary_provider", None) and ai_manager._primary_provider.is_available:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        try:
            ...
            async def dummy_cb(pct, msg): pass
            result = await scanner_engine.run_scan_with_progress(
                scan_path, job_id, None, ScanMode.LOCAL,
                timeout=480, progress_callback=dummy_cb, cloud_provider=provider
            )
        except Exception as e:
    ...
    header = f"Vexa Autonomous Agency — Remediation Report\n"
...
def main():
    ...
    print(f"Vexa MCP Server: Starting initialization (Version: {__version__})...", file=sys.stderr)
